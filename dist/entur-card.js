...
let EnTurCard = class EnTurCard extends LitElement {
    ...
    render() {
        if (['en', 'nb'].includes(this.hass.selectedLanguage)) {
            hooks.locale(this.hass.selectedLanguage);
        }
        else {
            hooks.locale('en');
        }
    ...
    isNowOrHasBeen(due_at) {
        const now = hooks();
        const when = hooks(due_at, 'HH:mm:ss');
        if (when > now) {
            return html `
        <span class="entur-human is-now">
          ${localize('common.arrives')} ${hooks(due_at, 'HH:mm:ss').fromNow()}
        </span>
      `;
        }
        ...
        <span class="entur-human has-been">
          ${localize('common.arrived')} ${hooks(due_at, 'HH:mm:ss').fromNow()}
        </span>
      `;
        }
        else {
